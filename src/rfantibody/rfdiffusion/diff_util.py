...
def process_ss(ss, xyz, bb_mask, fb=False):
    """
    Wrapper to process ss dict
    NEW. block_adj = 2 is masked
    """
    # filter out edge cases immediately
    compatible=False
    if ss is not None:
        if fb:
            if len(ss['ss']) == xyz.shape[0]:
                compatible=True
        else:
            if len(ss['ss']) == torch.sum(bb_mask):
                compatible=True
        if compatible is True:
            ss = ss_to_tensor(ss)
            if fb:
                ss=ss[bb_mask]
            block_adj = construct_block_adj_matrix(torch.from_numpy(np.copy(ss)).float(), xyz, bb_mask)
            ss_tens, mask = mask_ss(ss)
            masked_adj = torch.clone(block_adj)
            masked_adj[mask,:] = 2
            masked_adj[:,mask] = 2
        else:
            #mask whole ss and block adjacency
            L_res = torch.sum(bb_mask)
            ss_tens = torch.zeros((L_res,4))
            ss_tens[:,3] = 1
            masked_adj = torch.full((L_res,L_res), 2) # block adj mask token

    else:
        print('skipped monomer ss, fb=',fb)
        #mask whole ss and block adjacency
        L_res = torch.sum(bb_mask)
        ss_tens = torch.zeros((L_res,4))
        ss_tens[:,3] = 1
        #TODO make this mask token
        masked_adj = torch.zeros((L_res,L_res))
    return ss_tens, masked_adj

def process_complex_ss(ssA, ssB, xyz, bb_mask):
    """
    Wrapper to process two ss dicts
    """
    #filter out all edge cases
    if ssA is not None and ssB is not None and len(ssA['ss']) + len(ssB['ss']) == torch.sum(bb_mask):

        joint_dict = {}
        for key in ssA.keys():
            if key != 'idx':
                joint_dict[key] = ssA[key] + ssB[key]
            else:
                joint_dict[key] = ssA[key] + [i + ssA[key][-1] for i in ssB[key]]

        ss = ss_to_tensor(joint_dict)
        block_adj = construct_block_adj_matrix(torch.from_numpy(np.copy(ss)).float(), xyz, bb_mask)
        ss_tens, mask = mask_ss(ss)
        masked_adj = torch.clone(block_adj)
        masked_adj[mask,:] = 2
        ...
        # 50% of the time, mask inter-chain adjacency
        if random.uniform(0,1) < 0.5:
            A_len = len(ssA['ss'])
            masked_adj[:A_len,A_len:] = 2
            masked_adj[A_len:, :A_len] = 2
    else:
        print('skipped complex ss')
        #mask whole ss and block adjacency
        L_res = torch.sum(bb_mask)
        ss_tens = torch.zeros((L_res,4))
        ss_tens[:,3] = 1
        masked_adj = torch.full((L_res,L_res), 2)
