...
class PotentialManager:
    ...
    def parse_potential_string(self, potstr):
        ...
        '''
        str_settings  = ['type', 'point_def']
        bool_settings = ['glycan_squared_dist']

        setting_dict = {entry.split(':')[0]:entry.split(':')[1] for entry in potstr.split(',')}

        for key in setting_dict:

            if key in str_settings: continue
            elif key in bool_settings: setting_dict[key] = setting_dict[key] == 'True'
